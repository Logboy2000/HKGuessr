/**
 * Image Pack Editor - State Management and UI Synchronization
 * 
 * This script manages the state of image packs and ensures the UI stays
 * synchronized with the state data.
 */
...
function updateGameModeList() {
	...
	STATE.gameModes.forEach((mode) => {
		...
		div.className = 'list-item' + (mode === STATE.currentGameMode ? ' selected' : '');
...
function updateJsonPreview() {
	...
	const locationsForJson = transformLocationsForExport(STATE.currentGameMode.locations);
...
function updateLocationList() {
	...
	if (STATE.currentGameMode.locations.length === 0) {
		...
		emptyMessage.textContent = 'No locations added yet. Add your first location using the form.';
	...
	STATE.currentGameMode.locations.forEach((loc, index) => {
		const div = document.createElement('div');
		div.className = 'list-item' + (index === STATE.selectedLocationIndex ? ' selected' : '');
...
function addLocation() {
	...
	const difficulty = parseInt(document.getElementById('locationDifficulty').value);
...
function updateLocation() {
	...
	const oldLocation = STATE.currentGameMode.locations[STATE.selectedLocationIndex];
...
async function loadFile(path) {
	try {
		...
		document.getElementById('modeList').innerHTML = '<span>Loading image pack...</span>';
...
async function loadLocalFile(event) {
	...
	try {
		...
		if (file.name.endsWith('.zip')) {
			...
			if (imagesFolder) {
				...
				for (const location of data.locations) {
					...
					const imageFile = imagesFolder.file(`images/${imageName}`) || imagesFolder.file(imageName);
		...
		const existingModeIndex = STATE.gameModes.findIndex(mode => mode.gameModeId === data.gameModeId);
			...
			if (uploadedImage) {
				...
				try {
					...
					if (response.ok) {
						...
						console.warn(`Could not fetch image: ${imagePath} - HTTP ${response.status}`);
					}
				} catch (error) {
...
function updateCurrentGameModeFromForm() {
	...
	STATE.currentGameMode.gameModeId = document.getElementById('gameModeId').value;
	STATE.currentGameMode.author = document.getElementById('gameModeAuthor').value;
...
window.addEventListener('beforeunload', (event) => {
	...
	const hasUnsavedChanges = STATE.gameModes.some(mode => mode.saved === false);
