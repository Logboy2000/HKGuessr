...
const imagePacks = ['normal', 'charms']
...
async function loadCustomImagePack(file) {
    try {
        ...
        const locations = await Promise.all(data.locations.map(async (loc) => {
        ...
        if (typeof usedLocations !== 'undefined') {
            ...
            console.warn('usedLocations not initialized yet, waiting for script.js to load')
            // Create a small delay to allow script.js to initialize
            await new Promise(resolve => setTimeout(resolve, 100))
            if (typeof usedLocations !== 'undefined') {
                usedLocations[data.gameModeId] = []
            } else {
                throw new Error('usedLocations is not available. Make sure script.js is loaded first.')
            }
        }
	...
	try {
		...
		for (const packName of imagePacks) {
			try {
				...
				const response = await fetch(`imagePacks/${packName}/pack.json`)
				if (!response.ok) {
				...
				const locations = data.locations.map((loc) => {
                    ...
                    return [
                        ...
                        loc.y,
                        loc.difficulty,
                        `imagePacks/${packName}/images/${imageName}`,
                    ]
                })
